...
import { TEMPLE_GATES, VX, VZ, worldGateOpen } from "../world3d/village";
import { POND, vWorld } from "../world3d/field";
...
export function CompassRose() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const yaw = live.yaw;
      if (rose.current) rose.current.style.transform = `rotate(${(-yaw * 180) / Math.PI}deg)`;
      const dx = VX - live.x;
      const dz = VZ - live.z;
      const ang = Math.atan2(-dx, -dz);
      if (needle.current) needle.current.style.transform = `rotate(${((ang - yaw) * 180) / Math.PI}deg)`;
  ...
  return (
    <div
      className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full border-2 border-[#c9a227] bg-[#1a2418] shadow-md"
      aria-label="Compass. Gold needle points to Oakstead."
      title="Gold needle · Oakstead"
    >
      <div ref={rose} className="absolute inset-0">
        <span className="absolute top-0.5 left-1/2 -translate-x-1/2 text-[8px] font-bold leading-none text-[#e8d48a]">
          N
        </span>
        <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 text-[7px] leading-none text-[#8a8070]">
          S
        </span>
        <span className="absolute top-1/2 right-0.5 -translate-y-1/2 text-[7px] leading-none text-[#8a8070]">
          E
        </span>
        <span className="absolute top-1/2 left-0.5 -translate-y-1/2 text-[7px] leading-none text-[#8a8070]">
          W
        </span>
      </div>
      <div ref={needle} className="absolute inset-0 flex items-start justify-center pt-1">
        <span className="block h-3.5 w-0 border-l-[5px] border-r-[5px] border-b-[10px] border-l-transparent border-r-transparent border-b-[#e8c040]" />
      </div>
    </div>
  );
}
...
const MAP_MARKS: { id: string; label: string; x: number; z: number; kind: "town" | "keep" | "shrine" | "cave" | "water" | "gate" }[] = [
  { id: "oak", label: "Oakstead", x: 0, z: -188, kind: "town" },
  { id: "keep", label: "Keep", x: 0, z: -26, kind: "keep" },
  { id: "shrine", label: "Shrine", x: -82, z: 12, kind: "shrine" },
  { id: "cave", label: "Cavern", x: 22.6, z: 9.4, kind: "cave" },
  { id: "pond", label: "Pond", x: POND.x, z: POND.z, kind: "water" },
  { id: "loft", label: "Loft", x: LOFT.x, z: LOFT.z, kind: "town" },
  { id: "cedar", label: "Cedar", x: 18.6, z: -26.4, kind: "keep" },
  { id: "mill", label: "Mill", x: MILL.x, z: MILL.z, kind: "town" },
  { id: "pell", label: "Pell", x: PELL.x, z: PELL.z, kind: "town" },
];

const GATE_CHAIN = ["marsh", "grove", "crater", "lake", "grave", "waste", "echo", "ridge", "spire", "fen", "hollow", "vault"];

export function ValeMap({ you }: { you?: { x: number; z: number } }) {
  const cleared = useGame((s) => s.worldsCleared);
  const gems = useGame((s) => s.gems);
  const openGates = TEMPLE_GATES.filter((g) => worldGateOpen(g.world, cleared, gems));
  const youRef = useRef<HTMLSpanElement>(null);
  const pts = [
    ...MAP_MARKS.map((m) => ({ x: m.x, z: m.z })),
    ...openGates.map((g) => ({ x: g.x, z: g.z })),
  ...
  const roads = [
    road({ x: 0, z: -188 }, { x: 0, z: -26 }),
    road({ x: 0, z: -188 }, { x: -82, z: 12 }),
    road({ x: 0, z: -26 }, { x: 22.6, z: 9.4 }),
    road({ x: 0, z: -188 }, { x: POND.x, z: POND.z }),
    road({ x: 0, z: -26 }, { x: 18.6, z: -26.4 }),
    road({ x: PELL.x, z: PELL.z }, { x: LOFT.x, z: LOFT.z }),
    road({ x: 0, z: -188 }, { x: MILL.x, z: MILL.z }),
    road({ x: MILL.x, z: MILL.z }, { x: PELL.x, z: PELL.z }),
    road({ x: 0, z: -188 }, { x: -96, z: -172 }),
  ];
  if (openGates.some((g) => g.world === "marsh")) roads.push(road({ x: 0, z: -20 }, { x: -48, z: 88 }));
  if (openGates.some((g) => g.world === "grove")) roads.push(road({ x: 22.6, z: 9.4 }, { x: 168, z: 40 }));
  const chainPts = GATE_CHAIN.map((id) => openGates.find((g) => g.world === id)).filter(Boolean) as typeof TEMPLE_GATES;
  for (let i = 1; i < chainPts.length; i++) roads.push(road(chainPts[i - 1]!, chainPts[i]!));
  const blobs: { x: number; z: number; r: number; color: string }[] = [
    { x: 0, z: -188, r: 28, color: "rgba(90,130,56,0.42)" },
    { x: 0, z: -26, r: 16, color: "rgba(90,86,78,0.5)" },
    { x: POND.x, z: POND.z, r: 14, color: "rgba(48,110,140,0.48)" },
    { x: -82, z: 12, r: 14, color: "rgba(180,150,48,0.32)" },
    { x: LOFT.x, z: LOFT.z, r: 10, color: "rgba(140,90,50,0.35)" },
  ];
  for (const g of openGates) blobs.push({ x: g.x, z: g.z, r: 18, color: `${g.color}55` });
  ...
  return (
    <div className="vale-map relative overflow-hidden rounded-lg border-2 border-[#c9a227]/70">
      ...
      <span className="absolute top-1 left-1/2 -translate-x-1/2 text-[9px] font-bold tracking-[0.28em] text-[#e8d48a]">N</span>
      {MAP_MARKS.map((m) => (
        <span
          key={m.id}
      ...
      {openGates.map((g) => {
        ...
        return (
          ...
          >
            ◇ {g.name.replace(/^(The |Product |Factor |Quotient |Shadow |Spirit |Proof |Hour |Mirror |Storm |Crown |Carry )/, "")}
          </span>
        );
      })}
      <span
